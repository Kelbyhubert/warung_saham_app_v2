...
const StockTable : React.FC<StockTableProps> = ({currentIndex,currentLimit}) => {
  ...
  React.useEffect(() => {
    const fetchInsightData = async() => {
      try {
        const response = await fetch(`http://localhost:5173/api/stock?index=${currentIndex}&limit=${currentLimit}&search=${stockCode}`, {
          method: 'GET',
          credentials: 'include'
        });
