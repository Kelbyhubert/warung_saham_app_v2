    ...
    const fetchInsightData = async () => {
      try {
        const response = await fetch(
          `http://localhost:5173/api/insight?index=${currentIndex}&limit=${currentLimit}&title=${title}&createby=${createBy}&startdate=${startDate}&enddate=${endDate}`,
          {
            method: "GET",
            credentials: "include",
          }
  ...
  return (
    <div>
      <Table<InsightTableData> data={insights} totalData={1}>
        ...
        <Column
          body={(data) => dateFormatV1(data.createDate)}
          header="Create At"
        />
        <Column field="updateBy" header="Update By" />
        <Column
          body={(data) => dateFormatV1(data.updateDate)}
          header="Update At"
        />
        <Column
