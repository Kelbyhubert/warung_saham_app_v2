...
const TargetTable: React.FC<TargetTableProps> = (props) => {
  ...
  const statusTemplate = (data: TargetTableData, index: number | undefined) => {
    return (
      <TextField
        type="checkbox"
        key={"edit_status_" + index}
        disabled={index === editModeIndex}
      ...
      <div className="flex justify-center gap-2">
        {editModeIndex === index ? (
          ...
          <>
            ...
            <Trash2Icon
              onClick={() => deleteHandler(index === undefined ? -1 : index)}
            />
          </>
        )}
      </div>
  ...
  return (
    <div>
      ...
      >
        ...
        <Column
          header="Target Range"
          body={(data, index) => targetTemplate(data, index)}
        />
        <Column
          header="Order"
          body={(data, index) => orderTemplate(data, index)}
        />
        <Column
          header="Status"
          body={(data, index) => statusTemplate(data, index)}
        />
        <Column
          header="Action"
