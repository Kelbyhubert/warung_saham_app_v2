...
const AddTargetForm: React.FC<AddTargetFormProps> = (props) => {
  ...
  const submitHandler = () => {
    props.onSubmit(
      targetFromInputRef.current?.value,
      targetToInputRef.current?.value
    );
    props.onClose();
  };
  ...
  return (
    <Dialog onClose={onCloseHandler}>
      ...
      <DialogContent>
        <TextField label="Target From" ref={targetFromInputRef} type="number" />
        <TextField label="Target To" ref={targetToInputRef} type="number" />
        <TextField label="Order" ref={orderInputRef} type="number" />
      </DialogContent>
      <DialogAction>
        <button
